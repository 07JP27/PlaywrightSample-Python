...
def main():
    runner = TestRunner("test_11_multi_page")
    with sync_playwright() as p:
        ...
        for test_func in [
            test_new_tab,
            test_context_expect_page,
            test_enumerate_all_pages,
            test_bring_to_front,
            test_cross_page_data_flow,
            test_close_page_and_verify,
        ]:
        ...
        # page-only tests
        for test_func in [
            test_expect_popup_with_target_blank,
            test_window_open_popup,
        ]:
            context = browser.new_context(
                viewport={"width": 1280, "height": 720},
                locale="ja-JP",
        ...
        # page+context tests
        for test_func in [test_window_open_with_url]:
            context = browser.new_context(
                viewport={"width": 1280, "height": 720},
                locale="ja-JP",
                timezone_id="Asia/Tokyo",
            ...
            page = context.new_page()
            runner.run(test_func, page, context)
            context.close()
