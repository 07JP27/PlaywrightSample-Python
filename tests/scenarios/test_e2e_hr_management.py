...
def main():
    runner = TestRunner("test_e2e_hr_management")
    session_id = __import__("datetime").datetime.now().strftime("%Y%m%d_%H%M%S")
    all_evidence = []

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        instance = TestHRManagementFlow()

        test_methods = [
            instance.test_admin_login_and_dashboard,
            instance.test_navigate_to_pim_module,
            instance.test_search_employee,
            instance.test_dashboard_menu_navigation,
            instance.test_login_logout_cycle,
            instance.test_invalid_login_attempt,
        ]
