...
> **Note:** 本プロジェクトは pytest を使用せず、純粋な Playwright + Python スクリプトで構成されています。  
> 各テストファイルは `python tests/features/test_XX.py` で直接実行できます。
...
```bash
# 特定テストファイルの実行
python tests/features/test_01_browser_launch.py

# 機能別サンプルを一括実行
for f in tests/features/test_*.py; do python "$f"; done

# E2Eシナリオテストの実行
python tests/scenarios/test_e2e_ec_purchase.py

# 全シナリオを一括実行
for f in tests/scenarios/test_e2e_*.py; do python "$f"; done
```
...
│       ├── evidence.py              # エビデンス記録・HTMLレポート生成
...
「UI の見た目が意図せず変わっていないか検知したい」ときに参照。expect(page).to_have_screenshot() でベースライン画像と比較し、ピクセル差分を自動検出する。
